...
function drawLine(
    ...
    if (chartLine.isAutoScale) {
        const values = getFilteredData(
            chartLine,
            chartStructure.chart.timeDomain as Date[]
        );
        const verticalDomain = getVerticalDomain(values);
        const updatedVerticalScale = d3
            .scaleLinear()
            .domain(verticalDomain)
            .range([
                chartStructure.chart.horizontalAxis.offset.top,
                chartStructure.chart.getClientHeight() -
                    chartStructure.chart.horizontalAxis.offset.bottom,
                0,
            ]);
        const line = d3
            .line()
